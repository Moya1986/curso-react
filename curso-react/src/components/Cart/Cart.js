...
if(getQuantity() === 0) {
    ...
    return(
        <div>
            <h1>Cart</h1>
            <ul>
                {cart.map(prod =><li key={prod.id}>{prod.name} precio uni: {prod.price} cantidad: {prod.quantity} subtotal: {prod.price * prod.quantity} <button onClick={() => removeItem(prod.id)}>x</button></li>)}
            </ul>
